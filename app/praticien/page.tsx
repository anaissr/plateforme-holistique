...
Je suis spécialisée dans les troubles digestifs (ballonnements, intestin irritable, SIBO), la fatigue chronique, les déséquilibres hormonaux et la préparation à la grossesse.`,
    prestations: [
      { nom: 'Bilan initial', duree: '1h30', tarif: '110€', tarif_num: 110, description: 'Premier rendez-vous complet — anamnèse, bilan de vitalité, protocole personnalisé' },
      { nom: 'Consultation de suivi', duree: '45min', tarif: '70€', tarif_num: 70, description: 'Ajustement du protocole, bilan d étape, nouvelles recommandations' },
      { nom: 'Consultation visio', duree: '45min', tarif: '65€', tarif_num: 65, description: 'Consultation de suivi à distance, même qualité d accompagnement' },
    ],
    public: ['Adultes', 'Femmes enceintes', 'PMA'],
    specialites: ['Troubles digestifs', 'Fatigue chronique', 'Déséquilibres hormonaux', 'PMA et fertilité'],
    formations: [
    ...
    avisClients: [
      ...
      { prenom: 'Julie R.', date: 'Février 2026', note: 5, ponctualite: 5, ecoute: 5, clarte: 4, texte: 'Consultation en visio depuis Singapour, très facile à mettre en place.' },
      { prenom: 'Thomas B.', date: 'Janvier 2026', note: 4, ponctualite: 4, ecoute: 5, clarte: 5, texte: 'Très bon bilan, protocole clair et progressif.' },
    ],
  }
  ...
  const prendreRdv = async () => {
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        mode: prestation.nom.includes('visio') ? 'Visio' : 'Cabinet',
        statut: 'en_attente',
      ...
      <div className="max-w-7xl mx-auto px-12 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-white rounded-3xl shadow-sm overflow-hidden" style={{ border: '1px solid #e7e5e4' }}>
            ...
            {modePaiement === 'cadeau' && (
              <div className="p-6">
                ...
                />
                <button className="w-full text-white py-3 rounded-2xl text-sm font-medium" style={{ backgroundColor: '#6b21a8' }}>
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm" style={{ border: '1px solid #e7e5e4' }}>
            ...
            <p className="text-xs mb-3" style={{ color: '#a8a29e' }}>Échangez avec Sophie avant de vous engager</p>
            <textarea
